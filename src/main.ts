...
const createScene = async () => {
  ...
  try {
    const cabin = await loadGLTF(
      scene,
      '/assets/model/forest_cabin/',
      'forest_cabin12.glb',
      new Vector3(Tools.ToRadians(-90), Tools.ToRadians(145), 0)
    );
    if (cabin && cabin.root) {
  ...
  try {
    const anthill = await loadGLTF(
      scene,
      '/assets/model/anthill/',
      'animated_ant_hill.glb',
      new Vector3(Tools.ToRadians(0), Tools.ToRadians(-90), 0)
    );
    if (anthill && anthill.root) {
      ...
      anthillMesh = anthill.meshes.find((mesh) =>
        mesh.name.toLowerCase().includes('animated_ant_hill')
      ) as Mesh;
  ...
  try {
    await ant.load(
      ...
      'scene.gltf',
      0.05,
      new Vector3(Tools.ToRadians(-90), Tools.ToRadians(-90), 0)
    );
  ...
  try {
    await candyPirate.load(
      '/assets/model/candy_pirate/',
      'scene.gltf',
      0.8,
      new Vector3(Tools.ToRadians(-90), Tools.ToRadians(45), 0)
    );
    candyPirate.setPosition(new Vector3(8, 0, -8)); // Posición en una esquina del mapa

    console.log(
      '✓ Candy Pirate inteligente cargado exitosamente en:',
      candyPirate.getPosition().toString()
    );
  } catch (e) {
    console.error('Error cargando el Candy Pirate:', e);
  }

  // Paquetes (Candies) - Posiciones mejoradas y mejor distribuidas
  const paquetes: Mesh[] = [];
  const candyModels = ['sweetness1.glb', 'sweetness2.glb', 'sweetness3.glb', 'sweetness4.glb'];
  const positions = [
    new Vector3(8, 0.2, 12), // Cerca de la cabaña, lado derecho
    new Vector3(-15, 0.2, 8), // Lado izquierdo, distante
    new Vector3(12, 0.2, -12), // Esquina inferior derecha
    new Vector3(-8, 0.2, -18), // Esquina inferior izquierda, alejado del hormiguero
  ];

  for (let i = 0; i < positions.length; i++) {
    try {
      const candy = await loadGLTF(
        scene,
        '/assets/model/candies/',
        candyModels[i % candyModels.length],
        new Vector3(Tools.ToRadians(-90), 0, 0)
      );
    ...
    if (e.key.toLowerCase() === 'f') {
      if (!paqueteEnMano) {
        ...
        if (anthillRoot) {
          ...
          console.log(
            `Distancia al hormiguero: ${distanceToAnthill.toFixed(2)} | Ant: ${ant
              .getPosition()
              .toString()} | Anthill: ${anthillPosition.toString()}`
          ); // Debug
          ...
          if (distanceToAnthill < 1.5) {
            ...
            console.log(
              `¡Debes acercarte al hormiguero! Distancia actual: ${distanceToAnthill.toFixed(2)}`
            ); // Debug
          }
        } else {
  ...
  console.log('================');
  console.log('🔒 SISTEMA ULTRA SIMPLIFICADO ACTIVO');
  console.log('� Física compleja DESACTIVADA');
  console.log('� Altura fija SIN detección de suelo');
  console.log('⚡ Movimiento directo sin colisiones');
  console.log('================');
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    // SISTEMA SIMPLIFICADO: Altura fija sin detección de suelo
    const currentPos = ant.getPosition();
    const fixedHeight = 0.2; // Altura fija para la hormiga
    ...
    switch (currentCameraMode) {
      ...
      case CameraMode.FREE:
        ...
        if (inputMap['w'])
          camera.position.addInPlace(camera.getForwardRay().direction.scale(cameraSpeed));
        if (inputMap['s'])
          camera.position.addInPlace(camera.getForwardRay().direction.scale(-cameraSpeed));
        if (inputMap['a'])
          camera.position.addInPlace(
            Vector3.Cross(camera.getForwardRay().direction, Vector3.Up()).scale(-cameraSpeed)
          );
        if (inputMap['d'])
          camera.position.addInPlace(
            Vector3.Cross(camera.getForwardRay().direction, Vector3.Up()).scale(cameraSpeed)
    ...
    const distanceToHome = anthillRoot
      ? Vector3.Distance(ant.getPosition(), anthillRoot.position)
      : 0;
